...
> **现在由 `site-agent.yml` 自动化**（`installation_method: Agent-based`）。`tasks/iso_agent.yml`
> 渲染 install-config.yaml + agent-config.yaml + custom manifests 并调用
> `openshift-install agent create image`；OSS 上传 / ImportImage 复用 `02-import-image.yml`，
> 起实例复用 `06-create-cluster-stack.yml`。手工流程仅作原理参考。
> 网络走 **固定IP-via-DHCP + 空 hosts[]**（ROS 钉 master IP、cloud DHCP 下发;
> `scripts/abi-eni-spike.sh` 已确认这是阿里云上的干净路径,静态 NMState 会逼出 MAC 循环依赖)。
> agent ISO **在 mirror ECS 上构建**(VPC 内网):operator host 只渲染配置 + ssh 编排;mirror ECS 跑 `agent create image`(从本机 localhost:8443 拉 release)、经 coreos-installer(podman:show→jq merge→embed --force)注入 clone-vdb-to-vda hook、再用实例 RAM role 上传 OSS(内网 endpoint)+ ImportImage。
