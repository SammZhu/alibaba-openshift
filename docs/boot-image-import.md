...
- 决定好集群部署的 **Region**（如 `cn-hangzhou`）—— OSS 桶必须与 ROS 栈在**同一 Region**
...
3. 填写：
   ...
   - **OpenShift version**: 4.16+（External Platform 需要）
...
### 选项 B — Agent-based Installer

需要本地有 `openshift-install` 二进制：

```sh
# 下载 openshift-install（macOS 示例）
curl -L https://mirror.openshift.com/pub/openshift-v4/clients/ocp/latest/openshift-install-mac.tar.gz | tar -xz

# 准备 install-config.yaml（参考 ROS 栈 InstallConfig 输出）
...
记录下 OSS URL，形如：
```
https://openshift-images-myorg.oss-cn-hangzhou.aliyuncs.com/discovery-iso-cluster1.iso
```
...
# 创建 Bucket
aliyun oss mb oss://openshift-images-myorg --region cn-hangzhou
...
# 生成 URL（导入时使用）
echo "https://openshift-images-myorg.oss-cn-hangzhou.aliyuncs.com/discovery-iso-cluster1.iso"
```
