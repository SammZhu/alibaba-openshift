...
> clone-vdb-to-vda hook 已由 iso_agent 经 coreos-installer(show→jq merge→embed --force)注入 agent ISO live ignition。
> operator host 需装 `coreos-installer` + `jq` + `openshift-install`,且能访问 mirror registry(agent create image 本地拉 release)。
