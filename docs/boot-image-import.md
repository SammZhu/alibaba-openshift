...
> **现在由 `site-agent.yml` 自动化**（`installation_method: Agent-based`）。手工流程仅作原理参考。
> 编排(ENI-first via reimage,详见 docs/E2E-RUNBOOK.md):
> `06-create-cluster-stack`(phase-1:网络 + 用**占位镜像**起 master 拿到固定IP+自动主网卡,采主网卡 MAC)
> → `06a-build-agent-iso`(`tasks/iso_agent.yml` 渲染 install-config + agent-config[hosts[] 绑 MAC↔hostname] + manifests,调 `openshift-install agent create image`,**OSS 上传 + ImportImage 都折叠在这一步**,无独立 02)
> → `06b-reimage-masters`(phase-2:`UpdateStack` 把 ImageId 换成 agent 镜像,ROS 走 in-place **ReplaceSystemDisk** 保留实例/ENI/MAC/IP,master 重启进 agent 镜像自装)
> → `07-install-cluster-agent`(`agent wait-for` 监控 + 重写 *.apps + 取 kubeconfig)。
> 为何 ENI-first + reimage:hosts[] 要 MAC 绑定才能让 node-zero 拿到确定性主机名,而 MAC 必须造 ISO 前已知;ROS 又不支持把预建 ENI 设为主网卡,故用占位镜像先拿 MAC、再换盘。
> 装机 **直接落 /dev/vda**(无 vdb、无 clone-hook:minimal ISO 的 rootfs 跑在 RAM);rootfs 走 mirror ECS 内网 `:8080`(气隙)。
> agent ISO **在 mirror ECS 上构建**(VPC 内网):operator host 只渲染配置 + ssh 编排;mirror ECS 跑 `agent create image`(从本机 localhost:8443 拉 release)、用实例 RAM role 上传 OSS(内网 endpoint)+ ImportImage。
> 因此 ABI 前置:mirror 必须先起来;operator host 只需 ssh/scp + jq。
