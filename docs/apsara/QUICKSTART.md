...
⚠ **这条捷径只对同一个 `openshift_version` 成立。** 镜像内容和快照都是**按版本**的。
改了版本就必须重跑 `mirror-build`(tarball 按版本命名),而快照 03 会自己拒绝 ——
它记着自己是为哪个版本拍的,对不上就退回完整导入并说明原因。(在那之前,它会
安安静静地把旧版本的 Quay 恢复出来,而你要到 `06a/07` 节点拉不到镜像时才发现。)
