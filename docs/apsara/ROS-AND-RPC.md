# ROS templates and the apsara-rpc shim on Apsara Stack

> **Looking for how to install?** Start at [QUICKSTART.md](QUICKSTART.md) — the
> zero-to-cluster guide (one config file, one command).  This file is the
> reference underneath it: why the templates are transformed, what the
> per-service endpoint/scheme table looks like, and how to drive ROS by hand.
