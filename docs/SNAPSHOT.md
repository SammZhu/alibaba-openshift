...
- **fast-path 不重打 snapshot（P3-COST.7）**：05 只在本次**真的重建了 mirror
  内容**时才打 snapshot。纯 fast-path 恢复（03 snapshot-restore + 04 命中
  mirror-ready marker、跳过 import）下 vdb 跟恢复用的种子 snapshot 一致，05 跳过
  re-snapshot，`mirror_snapshot_data_id` 继续指向现有种子。信号：04 按
  `already_ready.rc` 设 `mirror_imported`（fast-path=false），05 门控
  `mirror_imported | default(true)`。这避免了"恢复用的旧种子 usage=disk 删不掉
  （P3-FIX.7）+ 新打一个 → 两个 ~200G 同名 snapshot 并存"。
