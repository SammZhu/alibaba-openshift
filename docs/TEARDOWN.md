...
| 资源 | 删？ |
|---|---|
| `aliocp1-cluster` ROS stack（VPC 用 mirror 的；含 3 master + NLB + SG + PrivateZone DNS 记录）| ✅ |
| state.yml 里 `cluster_*` 字段（`cluster_stack_id`、`api_lb_endpoint`、`worker_sg`、`master_ip*`、`*_zone_id*`）| ✅ |
| mirror-stack | ❌ |
| AI cluster + infra-env | ❌ |
| ECS image（Phase 02 build 的） | ❌ |
| mirror snapshot（vda + vdb）| ❌ |

**用途**：迭代 cluster install。常见场景。重建只需 06 → 07（约 1 小时）。
...
### 场景 C：换 OCP 版本（OCP 4.20.x → 4.21）

新 OCP = 新 RHCOS discovery agent 版本 = 新 ECS image 必须重建 = 旧 snapshot 也没用。
...
# 改 group_vars/all.yml 里 openshift_version 4.21.x，改 imageset-config.yaml 也行
