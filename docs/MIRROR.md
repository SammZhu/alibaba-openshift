...
```
 ...
 │                                                                     │
 │  04-prepare-mirror.yml (ansible 通过 SSH 跑重活):                   │
...
│       ├── 03-create-stack-LEGACY.yml              # 等 cloud-init + scp CA + PATCH install-config + infra-env
...
ansible-playbook ansible/playbooks/03-create-stack-LEGACY.yml        # ~5 min（bare stack，不含 mirror setup）
ansible-playbook ansible/playbooks/04-prepare-mirror.yml     # ~25-40 min（mirror-registry + import）
ansible-playbook ansible/playbooks/05-verify-mirror.yml      # ~30 sec（健康检查）
ansible-playbook ansible/playbooks/07-install-cluster.yml     # ~30 min
```
...
ansible-playbook ansible/playbooks/03-create-stack-LEGACY.yml   # ~30 min（cloud-init 再跑一遍）
ansible-playbook ansible/playbooks/05-verify-mirror.yml
...
| `connection refused` | mirror ECS 没起来 | `ansible-playbook 05-verify-mirror.yml` |
...
**相关文档**：
- [QUICKSTART.md](../QUICKSTART.md) — split flow 简版入口
- [ansible/README.md](../ansible/README.md) — split flow 完整命令 + 故障排查
- [docs/SNAPSHOT.md](SNAPSHOT.md) — mirror vda+vdb 快照生命周期 / fast-path / 销毁互斥
- [docs/TEARDOWN.md](TEARDOWN.md) — 销毁矩阵（`teardown_target × _preserves_ai × delete_snapshots`）
- [scripts/build-mirror-tarball.sh](../scripts/build-mirror-tarball.sh) （脚本顶部注释含 inline 用法）
- [ros-templates/mirror-stack.yaml](../ros-templates/mirror-stack.yaml) — 当前 split flow 的持久 mirror stack
- [ros-templates/create-cluster-LEGACY.yaml](../ros-templates/create-cluster-LEGACY.yaml) — 单栈 LEGACY 流程，搜 `Mirror` 看资源定义
...
| `manifest unknown` 拉 `rhai/...-agent-rhel9:<40-char hex>` | isc 把 tag 解析成 digest 后 oc-mirror v2 只 mirror digest，AI agent.service hard-code commit-tag → mirror 里没有这个 tag | build 脚本写 tag-mapping.tsv 到 OSS sibling；04 import 后 podman pull+tag+push 把 digest 别名成 commit-tag |
| `Failed to fetch container images needed for installation from quay.io/openshift-release-dev/ocp-release:4.20.23-x86_64` | `openshift_version` 用了 channel form ("4.20")，AI 自动 resolve 成 latest patch (4.20.23) 跟 mirror 实际版本 (4.20.22) drift | 在 `group_vars/all.yml` 把 `openshift_version` 钉死成 specific patch (`"4.20.22"`)；04 每次跑都会 assert + PATCH AI cluster.openshift_version；build-mirror-tarball.sh 的 OPENSHIFT_VERSION env 也用同一值 |
...
ansible-playbook 03-create-stack-LEGACY.yml
ansible-playbook 04-prepare-mirror.yml   # 在 existing data 上 idempotent
ansible-playbook 07-install-cluster.yml
```
