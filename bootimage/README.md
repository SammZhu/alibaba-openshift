...
|---|---|---|---|
| `bootimage/oldest-supported-minor` | **the FLOOR** — the oldest OCP minor Red Hat still supports normally (Full + Maintenance, **no EUS**). Refreshed from Red Hat's product lifecycle before every run by `scripts/rh_oldest_supported_minor.py`; hand edits are overwritten. Committed, because the runner has no local `all.yml`. NOT the list to bake; that set is derived at run time (floor→latest, minus provenance). | `rh_oldest_supported_minor.py` + git | ✅ |
