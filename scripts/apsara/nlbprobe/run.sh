...
#
#   NLB_ENDPOINT=nlb-vpc.cloud.dyz7.com ./run.sh > /home/claude/nlb-probe.txt 2>&1
#
...
export NLB_ENDPOINT=${NLB_ENDPOINT:-$(get ENDPOINT_NLB)}
export AK=${AK:-$(get capa_ak)}
export SK=${SK:-$(get capa_sk)}

: "${NLB_ENDPOINT:?set NLB_ENDPOINT (find it with scripts/apsara/probe-endpoints.sh)}"
: "${REGION:?could not read region from $AV}"
: "${AK:?could not read capa_ak from $AV}"

echo "region=$REGION  nlb=$NLB_ENDPOINT  ak=${AK:0:6}…"
...
echo "sdk:"; go list -m github.com/alibabacloud-go/nlb-20220430/v4 github.com/alibabacloud-go/darabonba-openapi/v2 | sed 's/^/  /'
echo
go build -o nlbprobe . && ./nlbprobe
