...
#
# ⚠ 固有局限:本脚本只能替换参考文件里已有的键。目标环境比参考环境**多出来**
# 的能力不会自动出现——它不知道有这么个键。实例:dyz7 没有 NAS,它的 all.yml
# 里连 ENDPOINT_NAS 这一行都没有,所以 ste2 的候选里也不会有,尽管 ste2 的
# NAS 是活的(nas-pub.<region>.cloud.<env>)。跑完对照一遍目标环境实际提供
# 的服务,补上参考环境没有的那些。
#
