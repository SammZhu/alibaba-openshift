# OpenShift on Alibaba Cloud — 端到端 QUICKSTART（LEGACY 单栈）

> ⚠️ **本指南是 LEGACY 单 stack 流程的手动操作手册**
> （`create-cluster-LEGACY.yaml` + `03-create-stack-LEGACY.yml` +
> `99-teardown-LEGACY.yml -e teardown_from=N`）。
>
> **推荐路径** = split 流程（mirror-stack + cluster-stack 解耦，cluster
> 重建快 ~30 min）：
> - 全自动 Ansible：[`ansible/README.md`](ansible/README.md)
> - 简要入口：[`QUICKSTART.md`](QUICKSTART.md)
> - 完整架构：[`README.md`](README.md)
>
> 命令对照速查：
>
> | LEGACY（本文档） | SPLIT（推荐） |
